...
for curr_sig_idx in range(num_real_sigs):
    ...
    for j in range(len(who)):
        ...
        print(selections_indexed_by_labeler)
        ...
        if selections_indexed_by_labeler[0] == -1 and selections_indexed_by_labeler[1] == -1:
            print(f"nosel made by {who[j]}: {selections_indexed_by_labeler}")
            no_labels += 1
        else:
            onsets[curr_sig_idx][j] = selections_indexed_by_labeler[0]
            offsets[curr_sig_idx][j] = selections_indexed_by_labeler[1]
...
valid_index_array = np.full(len(who),np.nan)
for i in range(num_real_sigs):
    spot_index = 0
    for j in range(len(who)):
        valid_index_array[j]=np.nan
    ...
    for j in range(len(who)):
        if onsets[i][j] >= lower_on and onsets[i][j] <= upper_on and offsets[i][j] >= lower_off and offsets[i][j] <= upper_off:
            valid_index_array[spot_index] = j
            spot_index += 1

    # print(f"valid indices for signal {i}: {valid_index_array}")

    center_onsets[i] = np.mean(onsets[i][~np.isnan(valid_index_array)])
    center_offsets[i] = np.mean(offsets[i][~np.isnan(valid_index_array)])
...
n_samples = 10
conf_analysis_results = np.zeros((n_samples, 2))
conf_value_index = 0
for conf_value in np.linspace(0.1,1, n_samples):
    for i in range(len(all_images)):
        image_path = all_images[i]
        # Predict results for the image
        results = model.predict(source=image_path, conf=conf_value)
    ...
    diff_onsets = center_onsets[valid_indices] - pred_onsets[valid_indices]
    diff_offsets = center_offsets[valid_indices] - pred_offsets[valid_indices]
    ...
    conf_analysis_results[conf_value_index][0] = np.abs(avg_diff_onsets)+np.abs(avg_diff_offsets)
    conf_analysis_results[conf_value_index][1] = find_rate
    conf_value_index += 1

fig0 = plt.figure(0)
ax0 = fig0.add_subplot(111)
ax0.set_title("conf value effect on MAE")
ax0.set_xlabel("confidence value")
ax0.set_ylabel("MAE")
plt.plot(np.linspace(0.1,1, n_samples), conf_analysis_results[:,0])

fig1 = plt.figure(1)
ax1 = fig1.add_subplot(111)
ax1.set_title("conf value effect on find rate")
ax1.set_xlabel("confidence value")
ax1.set_ylabel("find rate")
plt.plot(np.linspace(0.1,1, n_samples), conf_analysis_results[:,1])

fig2=plt.figure(2)
ax2 = fig2.add_subplot(111)
ax2.set_title("conf value effect on MAE as a percentage of signal")
ax2.set_xlabel("confidence value")
ax2.set_ylabel("MAE as a percentage of signal")
plt.plot(np.linspace(0.1,1, n_samples), 100 * conf_analysis_results[:,0]/910)
