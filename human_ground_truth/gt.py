...
data_reference_path = "../voyteklabstudy-default-rtdb-export.json"
# %%
...
# Load data
with open(data_reference_path) as f:
    results = json.load(f)
...
probability_pairs = np.empty((len(ground_truth), len(who), num_classes), dtype=object)
ground_truths = np.empty((len(ground_truth), len(who), num_classes), dtype=object)
sig_len_sim = len(results["sigs"]["sig_" + str(0)])
round_robin_array = np.zeros((len(ground_truth), sig_len_sim))
y_true = np.zeros((len(ground_truth), sig_len_sim))
...
# process for simulated signals.
# includes f1 score for demonstrating accuracy against ground truth.
...
for i in range(len(ground_truth)):
    ...
    for j in range(len(who)):
        ...
        for subIndex in range(
            selections_indexed_by_labeler[0], selections_indexed_by_labeler[1] + 1
        ):
    ...
    for k in range(len(eeg_signal_profiled_in_this_loop)):
        round_robin_array[i][k] = (
            1 if round_robin_array[i][k] >= consensus_threshold else 0
        )

    print(len(y_true[i]), len(round_robin_array[i]))
    scores[i] = sklearn.metrics.f1_score(y_true=y_true[i], y_pred=round_robin_array[i])

print(scores)

# process for real signals

real_round_robin = np.zeros((num_real_sigs, sig_len_sim))
consensus_ground_truth = np.zeros((num_real_sigs,2))
# iterates through each signal
for curr_sig_idx in range(num_real_sigs):
    ...
    for j in range(len(who)):
        order = np.array(results["selections"][who[j]]["indices"])
        selections = np.array(results["selections"][who[j]]["selections"])
        reverse_search_sig_idx = reverse_order(curr_sig_idx, order)
        selections_indexed_by_labeler = selections[reverse_search_sig_idx]
        ...
        y_pred_boolean = [False] * len_curr_sig
        for subIndex in range(
            selections_indexed_by_labeler[0], selections_indexed_by_labeler[1]
        ):
            print(selections_indexed_by_labeler[1])
            real_round_robin[curr_sig_idx][subIndex] += 1

    consensus_threshold = ceil(len(who) / float(2))
    for k in range(len(eeg_signal_profiled_in_this_loop)):
        real_round_robin[curr_sig_idx][k] = (
            1 if real_round_robin[curr_sig_idx][k] >= consensus_threshold else 0
        )

    from arbitrary_decisions import selections_to_burst
    ground_truth=selections_to_burst(real_round_robin, len_curr_sig/2.0)


    print(len(y_true[curr_sig_idx]), len(round_robin_array[curr_sig_idx]))
    # scores[i]=sklearn.metrics.f1_score(y_true=y_true[i], y_pred = round_robin_array[i])
    ...
    for labeler_index in range(len(who)):
        ...
        plt.axvspan(ground_truth[curr_sig_idx][0],
                    ground_truth[curr_sig_idx][1], color='blue', alpha=0.5)
