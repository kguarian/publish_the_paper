...
def optimize_dualthresh_params(signal):
    """
    Optimize the dual threshold parameters for a given signal.
    """
    # Placeholder for the actual optimization logic
    # For now, we just return some dummy parameters
    return [0.5, 0.5]
...
def parallel_subprocess_generate_ml_training_data(
    i,
    directory,
    n_sims,
    positive_sample,
    ...
    scaled_selection = scale_selection(ground_truth_sample)  # Create the annotation
...
def generate_ml_training_data(
    ...
    # 8 core processor. Should be better than 1
    p = Pool(8)
    for i in range(n_sims):
        p.apply_async(parallel_subprocess_generate_ml_training_data, (i,directory,n_sims, positive_examples[i], negative_examples[i], ground_truth[i]))
        # parallel_subprocess_generate_ml_training_data(i,directory,n_sims, positive_examples[i], negative_examples[i], ground_truth[i])
    p.close()
    p.join()
    # Generate YAML file
